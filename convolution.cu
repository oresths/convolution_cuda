...
__constant__ int s_mpF, s_npF, num_s_pF, quotient, modulo;


void random_initialization(float *matrixx, int m, int n){
	...
	for(i=0; i<m; i++){
		for(j=0; j<n; j++){
			matrixx[i*n+j] = (float)(rand()/(float)RAND_MAX) + (float)(rand()%5 - rand()%5);
		}

	}
}

void save(float *matrixx, int m, int n, char *filename){
	...
	fwrite(matrixx, sizeof(float), m*n, outfile);
...
void error_message(){
	...
	char *help = "Error using convolution: Four arguments required\n"
			"First: mF\n"
			"Second: nF\n"
			"Third: mT\n"
			"Fourth: nT\n";
...
void serial_convolution(float *F, float *T, float *pF, float *Y, int mF, int nF, int mT, int nT)
{
	int mpF=mT+mT+mF-2, npF=nT+nT+nF-2, mY=mF+mT-1, nY=nF+nT-1;
	int i, j, ii, jj;

	/**Perikleei ton F me ta mhdenika poy xreiazetai gia na ginei h convolution.**/
	for (i=0;i<mpF;i++){
		for (j=0;j<npF;j++){
			pF[i*npF+j]=0;
		}
...
__global__ void convolution(float *d_pF, float *d_Y)
{                          
	__shared__ float s_pF[4000];	//Xwros gia to kommati toy pF pou 8a paei sthn shared.
		

	int index=threadIdx.x*BLOCK_SIZE_Y+threadIdx.y;	//Xarakthristikos index gia ka8e nhma mesa sto mplok.
	int k=0;
	if (index<modulo) k=1;	//8elw ola ta nhmata na metaferoyn peripou iso ari8mo stoixeiwn toy pF sthn shared, alla epeidh h diairesh den einai-> 
	int transfer_count=quotient + k;	// ->teleia, ta prwta num_s_pF%mege8os_mplok nhmata 8a metaferoun ena parapanw stoixeio.
	int init_pos=transfer_count*index*k + ((quotient+1)*modulo+transfer_count*(index-modulo)) * (1-k);    //H 8esh apo opou ksekinaei h perioxh thn opoia 8a metaferei ka8e nhma.
	...
	int ii, jj;
	for (ii=init_pos;ii<init_pos+transfer_count;ii++)
...
int main(int argc, char **argv){
	...
	serial_convolution(F, T, pF, Y, mF, nF, mT, nT);   //Ftiaxnei kai ton pF poy xreiazetai kai apo thn cuda.
	...
	int h_num_s_pF=h_s_mpF*h_s_npF;	  //To plh8os twn stoixeiwn toy pinaka pF sthn shared.
	int h_quotient=h_num_s_pF/(BLOCK_SIZE_X*BLOCK_SIZE_Y);	   //Xreiazetai gia na ypologisw posa stoixeia toy pF 8a metaferei sthn shared ka8e nhma.
	int h_modulo=h_num_s_pF%(BLOCK_SIZE_X*BLOCK_SIZE_Y);	   //										>>

	float time_metaforas_eisodou;
	...
	cudaMemcpyToSymbol("num_s_pF", &h_num_s_pF, sizeof(int));
	cudaMemcpyToSymbol("quotient", &h_quotient, sizeof(int));
	cudaMemcpyToSymbol("modulo", &h_modulo, sizeof(int));
	
	cudaEventRecord(stop_event, 0);
	cudaEventSynchronize(stop_event);   // block until the event is actually recorded
	cutilSafeCall( cudaEventElapsedTime(&time_metaforas_eisodou, start_event, stop_event) );
	printf("\n\nXronos metaforas twn dedomenwn eisodoy sthn karta: %fsec\n", time_metaforas_eisodou/1000);
	...
	float time_synelikshs;
	cudaEventRecord(start_event, 0);     // record in stream-0, to ensure that all previous CUDA calls have completed

	// execute the kernel
	convolution<<< grid, threads >>>(d_pF, d_Y);


	cudaEventRecord(stop_event, 0);
	cudaEventSynchronize(stop_event);   // block until the event is actually recorded
	cutilSafeCall( cudaEventElapsedTime(&time_synelikshs, start_event, stop_event) );
	printf("time ypologismoy synelikshs: %fsec\n", time_synelikshs/1000);
